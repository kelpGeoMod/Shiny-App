...
x <- gstat::idw(i ~ 1, kelp_sf_2, newdata = grid_sf, idp = 2)
...
cropped_raster <- crop(y, extent(area_sf))
...
# crop the attribute_raster to the area of interest
sst_cropped_raster <- crop(y, extent(area_sf))
```
...
extent(model_raster) <- extent(kelp_raster_cropped)
crs(model_raster) <- crs(kelp_raster_cropped)
...
Check if the rasters have the same dimentions
```{r}

#compareRaster(model_raster, kelp_raster_cropped)
#plot(model_raster)
