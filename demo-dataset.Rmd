...
for (i in y_vars){
  ...
  test_final_cropped <- crop(y, extent(area_sf))
  
  paste0(i, "raster") <- final_cropped
  
  return(paste0(i, "raster"))
...
```{r}
  # interpolate with IDW method per attribute
x <- gstat::idw(NH4 ~ 1, kelp_sf_2, newdata = grid_sf, idp = 2)
...
# crop the attribute_raster to the area of interest
NH4_cropped_raster <- crop(y, extent(area_sf))
